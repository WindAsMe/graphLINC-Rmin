...
def graphFDMVM(N, func, pop_size, Max_iter_overlap, Max_iter_search, epsilon, overlap_ignore_rate, scale_range, cost):
    ...
    """
    initial_matrix = help.adjacent_matrix_initial(N, 'z')
    initial_connections = help.matrix_connection(initial_matrix)
    initial_groups = help.connections_groups(initial_connections)
    Population = help.random_Population(scale_range, N, pop_size)
    ...
    '''
    final_connection = help.matrix_connection(initial_matrix)
    ...
    help.draw_heatmap(initial_matrix, "YlGnBu", 'Best solution')
    final_groups = help.connections_groups(final_connection)
